...
import com.han.kafka.common.DefaultFlowRateLimiter;
...
import com.han.kafka.consumer.support.Handover;
 ...
 */
@Slf4j
public abstract class BaseKafkaConsumer extends Thread {
    ...
    public BaseKafkaConsumer(String threadName, Properties kafkaProperties, Long pollTimeout) {
        super(threadName);
    ...
    public BaseKafkaConsumer(String threadName, Properties kafkaProperties, Long pollTimeout, Double rateLimiter) {
        this(threadName, kafkaProperties, pollTimeout);
        this.rateLimiter = new DefaultFlowRateLimiter(rateLimiter);
    }

    public BaseKafkaConsumer(String threadName, Properties kafkaProperties, Long pollTimeout, Handover handover) {
        this(threadName, kafkaProperties, pollTimeout);
        this.handover = handover;
    }

    public BaseKafkaConsumer(String threadName, Properties kafkaProperties, Long pollTimeout, CommitCallback commitCallback) {
        this(threadName, kafkaProperties, pollTimeout);
        this.commitCallback = commitCallback;
    }

    public BaseKafkaConsumer(String threadName, Properties kafkaProperties, Long pollTimeout, Handover handover, CommitCallback commitCallback) {
        this(threadName, kafkaProperties, pollTimeout, handover);
        this.commitCallback = commitCallback;
    }
            ...
            while (running) {
                ...
                if (consumerEventQueue.size() > 0) {
                    try {
                        ...
                        reassingPartition(event.getTopicPartitionOffsets());
                    } catch (InterruptedException e) {
                        log.warn("reassingPartition failed:" + e);
                    }
                }
    ...
    public abstract void handleRecords(ConsumerRecords<String, String> records)throws AppException;
    ...
    KafkaConsumer<String, String> getConsumer(Properties kafkaProperties) {
        ...
        Set<String> topics = (Set<String>) kafkaProperties.get("consume.tpics");
        consumer.subscribe(topics);
        return consumer;
    }

    void reassingPartition(Map<String, List<PartitionOffset>> newPartitions) {
